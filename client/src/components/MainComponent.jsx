import axios, { all } from "axios";
import React, { useEffect, useState } from "react";
import { toast } from "react-toastify";
import { useDispatch, useSelector } from "react-redux";
import { setUserInfo } from "../store/userSlice";
import TweetBox from "./TweetBox";
import { getAllTweets, updateTweet } from "../store/tweetSlice";

const MainComponent = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [commentText, setCommentText] = useState("");

  const [tweet, setTweet] = useState({
    text: "",
    image: "",
  });

  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.user);
  const { allTweets } = useSelector((state) => state.tweet);

  useEffect(() => {
    let userInfo = JSON.parse(localStorage.getItem("userInfo"));
    if (userInfo) {
      dispatch(setUserInfo(userInfo));
    }
  }, [setUserInfo]);

  function convertToBase64(file) {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.readAsDataURL(file);
      reader.onload = () => {
        resolve(reader.result);
      };
      reader.onerror = (error) => {
        reject(error);
      };
    });
  }

  const handleImage = async (e) => {
    const file = e.target.files[0];
    const base64 = await convertToBase64(file);
    setTweet((prev) => ({ ...prev, image: base64 }));
  };

  const handleTweet = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8000/api/tweet",
        tweet,
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }
      );

      setTweet({
        text: "",
        image: "",
      });
      // Fetch updated tweet list
      fetchTweets();

      toast.success(response.data.message);
    } catch (error) {
      toast.error(error.response.message);
    }
  };

  const fetchTweets = async () => {
    try {
      setIsLoading(true);
      const sortedTweets = [];
      const response = await axios.get("http://localhost:8000/api/tweets", {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
      sortedTweets.push(...response.data.result);
      sortedTweets.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );
      dispatch(getAllTweets(sortedTweets));
      toast.success(response.data.message);
    } catch (error) {
      toast.error(error.response?.message);
    } finally {
      setIsLoading(false);
    }
  };

  const handleLike = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:8000/api/like/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }
      );
      const updatedTweets = allTweets.map((tweet) => {
        if (tweet._id === id) {
          return {
            ...tweet,
            liked: true, // set the liked property to true
            likes: [...tweet.likes, user._id],
          };
        } else {
          return tweet;
        }
      });
      // dispatch(updateTweet(updatedTweets));
      // console.log(updatedTweets);
      dispatch(getAllTweets(updatedTweets));
      toast.success(response.data.message);
      // fetchTweets();
    } catch (error) {
      toast.error(error.response.message);
    }
  };

  const handleUnLike = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:8000/api/unlike/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }
      );
      const updatedTweets = allTweets.map((tweet) => {
        if (tweet._id === id) {
          return {
            ...tweet,
            liked: false, // set the liked property to false
            likes: tweet.likes.filter((userId) => userId !== user._id),
          };
        } else {
          return tweet;
        }
      });
      // dispatch(updateTweet(updatedTweets));
      dispatch(getAllTweets(updatedTweets));
      toast.success(response.data.message);
      // fetchTweets();
    } catch (error) {
      toast.error(error.response.message);
    }
  };

  const handleComment = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:8000/api/comment/${id}`,
        { commentText },
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }
      );
      toast.success(response.data.message);
      setCommentText("");
      fetchTweets();
    } catch (error) {
      toast.error(error.response.message);
    }
  };
  const handleDelete = async (id) => {
    try {
      console.log(id);
      const response = await axios.delete(`http://localhost:8000/api/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
      toast.success(response.data.message);
      fetchTweets();
    } catch (error) {
      toast.error(error.response.message);
    }
  };

  useEffect(() => {
    fetchTweets();
  }, [user]);


  return (
    <section className="col-span-6 border border-gray-600 pt-8 px-3">
      {user && (
        <div className="flex flex-start gap-4">
          <div className="w-12 h-12 rounded-full">
            <img
              src={
                user?.profilePic
                  ? user?.profilePic
                  : "data:image/png;base64,"
              }
              alt={user?.name}
              className="w-full h-full rounded-full object-cover"
            />
          </div>
          <div className="flex flex-col w-full">
            <textarea
              placeholder="What's Happening?"
              className="bg-transparent p-3 text-white"
              value={tweet.text}
              maxLength={250}
              onChange={(e) =>
                setTweet((prev) => ({ ...prev, text: e.target.value }))
              }
            />
            <label htmlFor="tweet" className="w-max">
              <input
                type="file"
                onChange={handleImage}
                id="tweet"
                className="hidden"
              />
              <i className="fa-solid fa-image text-white text-2xl font-bold cursor-pointer mt-4"></i>
            </label>
            {tweet?.image && (
              <div className="h-96 w-full bg-green-600 rounded-xl mt-3">
                <img
                  src={tweet.image}
                  alt={tweet.text}
                  className="h-full w-full object-cover"
                />
              </div>
            )}
            <div className="flex items-center justify-end mt-4">
              <button
                onClick={handleTweet}
                disabled={!tweet.text}
                className="bg-[#1D9BF0] disabled:bg-[#3b83b3] hover:bg-[#0e89db] text-xl text-white px-12 py-2 text-bold rounded-full"
              >
                Tweet
              </button>
            </div>
          </div>
        </div>
      )}
      {isLoading ? (
        <div className="flex items-center justify-center w-full mt-12">
          <div className="w-12 h-12 rounded-full animate-spin border-4 border-solid border-blue-500 border-t-transparent"></div>
        </div>
      ) : (
        <div>
          {allTweets?.map((item) => {
            return (
              <TweetBox
                item={item}
                key={item?._id}
                handleLike={handleLike}
                commentText={commentText}
                setCommentText={setCommentText}
                handleUnLike={handleUnLike}
                handleComment={handleComment}
                handleDelete={handleDelete}
              />
            );
          })}
        </div>
      )}
    </section>
  );
};

export default MainComponent;
